...
export default function PrintToast({ qty, done, state, label, errorMsg, removing, onDismiss }: Omit<ToastState, 'id'> & { onDismiss: () => void }) {
  ...
  // The error variant is wider and grows vertically so the full message is readable.
  const shellBg = state === 'error'   ? 'bg-[#3d1a1a] border-[#f85149]'
                : state === 'success' ? 'bg-[#1a4731] border-[#2ea043]'
                :                       'bg-[#161b22] border-[#30363d]'
  const width = state === 'error' ? 'w-[320px]' : 'w-[230px]'
  const shell = `pointer-events-auto ${width} border rounded-xl px-3 py-[10px] shadow-[0_8px_28px_rgba(0,0,0,0.55)] animate-slide-up transition-[opacity,transform,background-color,border-color] duration-300 ${shellBg} ${fade}`

  if (state === 'error') return (
    <div className={shell}>
      <div className="flex items-start gap-2">
        <span className="text-[#f85149] shrink-0 leading-5">✗</span>
        <span className="text-[#ff7b72] text-xs flex-1 whitespace-pre-wrap break-words max-h-[40vh] overflow-y-auto leading-5">{errorMsg ?? 'Print failed'}</span>
