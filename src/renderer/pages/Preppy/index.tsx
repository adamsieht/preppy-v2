...
// ── Calendar picker (left-panel tab) ────────────────────────────────────────
...
function CalendarPicker({ template, onPrint, onCustomPrint }: CalendarPickerProps) {
  ...
  const stepBtn = 'w-10 h-10 rounded-lg border border-[#30363d] bg-[#161b22] text-white text-base cursor-pointer hover:bg-[#28a745]/10 hover:border-[#28a745] transition-colors flex items-center justify-center font-bold select-none'
  const timeVal = 'text-white font-bold text-xl w-8 text-center select-none'
  const timeLbl = 'text-[#6e7681] text-[10px] font-semibold uppercase tracking-widest text-center mb-1'
    ...
    <div className="flex flex-col flex-1 min-h-0 overflow-y-auto scrollbar-dark">
      ...
      <div className="flex items-end justify-center gap-4 px-3 py-3 border-t border-[#30363d] shrink-0">
        <div className="flex flex-col items-center">
          <div className={timeLbl}>Hour</div>
          <div className="flex items-center gap-2">
            <button className={stepBtn} onClick={() => setHour(h => h === 1 ? 12 : h - 1)}>−</button>
            <span className={timeVal}>{hour}</span>
            <button className={stepBtn} onClick={() => setHour(h => h === 12 ? 1 : h + 1)}>+</button>
          </div>
        </div>
        <span className="text-white font-bold text-xl mb-[2px]">:</span>
        <div className="flex flex-col items-center">
          <div className={timeLbl}>Min</div>
          <div className="flex items-center gap-2">
            <button className={stepBtn} onClick={() => setMinute(m => m === 0 ? 45 : m - 15)}>−</button>
            <span className={timeVal}>{minute.toString().padStart(2, '0')}</span>
            <button className={stepBtn} onClick={() => setMinute(m => m === 45 ? 0 : m + 15)}>+</button>
          </div>
        </div>
        <div className="flex flex-col items-center">
          <div className={timeLbl}>Period</div>
          <button
